...
import {collection, onSnapshot, updateDoc, doc, query, orderBy, getDocs, deleteDoc} from 'firebase/firestore';
...
const ContactDashboard = () => {
            ...
            try {
                ...
                const sortedSocials = socialOrder
                    ...
                    .map((social, index) => ({
                        ...
                        cover: social.socialCover ? social.socialCover : "/fallback-cover.jpg",
                        profile: social.socialProfile ? social.socialProfile : "/fallback-profile.jpg",
                    }));
    ...
    useEffect(() => {
        const packageIds = ["bsNLepR9f1Gj9yFenTbm", "q7MT2hanXirpRTUVMmDQ", "66r7pumc0E12VaXWXDfP"];
        const unsubscribe = onSnapshot(collection(db, "packages"), (snapshot) => {
            const data = snapshot.docs.map(doc => ({ id: doc.id, ...doc.data() }));
            const orderedData = packageIds.map(id => data.find(pkg => pkg.id === id)).filter(pkg => pkg);
    ...
    const handleSocialEdit = (id, social) => {
        ...
        setTimeout(() => {
            const container = document.querySelector(`.${styles["contactdashboard-socials-container"]}`);
            if (container) {
                const offset = -80;
    ...
    const handleSaveSocial = async (id) => {
        setSaving(id);
        try {
            const uploadPromises = Object.entries(selectedFiles).map(async ([field, file]) => {
                const options = {
                    maxSizeMB: 0.1,
                ...
                const storageRef = ref(storage, `images/contact/socials-uploads/${compressedFile.name}`);
    ...
    return (
        <section className={styles.dashboardContainer} style={{marginTop:"-40px"}}>
            <div className={styles.innerWrapper}>
                ...
                <h1 className={styles.title}
                    style={{fontWeight: "900", textShadow: "0 0 1px black, 0 0 1px black, 0 0 1px black"}}>Contact Us -
                    Update Packages & Rates</h1>
                <table className={styles.table} style={{width: "800px"}}>
                    ...
                    {packages.map((pkg) => (
                        <tr key={pkg.id} className={styles.tableRow}>
                            <td>
                                {editingRow === pkg.id ? (
                                    <input type="text" defaultValue={pkg.packageType} className={styles.input}
                                           id={`type-${pkg.id}`}/>
                                ) : (
                                    pkg.packageType
                                )}
                            ...
                            <td>
                                {editingRow === pkg.id ? (
                                    <input type="text" defaultValue={pkg.packagePrice} className={styles.input}
                                           id={`price-${pkg.id}`}/>
                                ) : (
                                    pkg.packagePrice
                                )}
                            ...
                            <td>
                                {editingRow === pkg.id ? (
                                    <div className={styles.buttonGroup1}>
                                        <button className={`${styles.button1} ${styles.gray}`}
                                                onClick={() => setEditingRow(null)}><i className="fa fa-close"
                                                                                       style={{marginRight: "5px"}}></i>Cancel
                                        </button>
                                        <button
                                            className={`${styles.button1} ${styles.green}`}
                                            onClick={() => {
                                                const updatedType = document.getElementById(`type-${pkg.id}`).value;
                                                const updatedPrice = document.getElementById(`price-${pkg.id}`).value;
                                        ...
                                        >
                                            {saving === pkg.id ? (
                                                ...
                                                <>
                                                    Save
                                                    <i className="fa fa-check-circle-o" style={{marginLeft: "5px"}}></i>
                                                </>
                                            )}
                                        </button>
                                    ...
                                    <button className={`${styles.button1} ${styles.orange}`}
                ...
                }}>Contact Us - Customer Inquiries</h1>
                        ...
                        [...contactMessages]
                            ...
                            .map((msg) => (
                                ...
                                >
                                    ...
                                    <td>{msg.inquiryType === "other" ? msg.otherInquiryType : msg.inquiryType}</td>
                                    ...
                                    <td>
                                        {!msg.isRead ? (
                                            ...
                                            >
                                                <i className="fa fa-check" style={{marginRight: "5px"}}></i>Mark as
                                                Read
                                            </button>
                                        ) : (
                                            <button
                                            ...
                                            >
                                                <i className="fa fa-undo" style={{marginRight: "5px"}}></i>Mark as
                                                Unread
                                            </button>
                                        )}
                                        <button
                                        ...
                                        >
                                            Delete<i className="fa fa-trash" style={{marginLeft: "5px"}}></i>
                                        </button>
                                    </td>
                                </tr>
                            ))
                        ...
                        socials.map((social) => (
                            <div
                                ...
                                className={`${styles["contactdashboard-social-box"]} ${
                                    editingSocialId === social.id ? styles["contactdashboard-social-box-editing"] : ""
                                }`}
                            >
                                {editingSocialId === social.id ? (
                                    <div className={styles["contactdashboard-edit-container"]}>
                                        ...
                                        <div className={styles["contactdashboard-edit-file"]}
                                            ...
                                            <label style={{marginTop: "20px"}}>Social Media Link:</label>
                                        ...
                                        <div className={styles["contactdashboard-button-group"]}>
                                            <button className={`${styles.button1} ${styles.gray}`}
                                                    onClick={handleCancelEdit} disabled={saving === social.id}><i
                                                className="fa fa-close" style={{marginRight: "5px"}}></i>Cancel
                                            </button>
                                            <button
                                                className={`${styles.button1} ${styles.green}`}
                                            ...
                                            >
                                                {saving === social.id ? (
                                                    ...
                                                    <>
                                                        Save<i className="fa fa-check-circle-o"
                                    ...
                                    <>
                                        ...
                                        <button style={{marginTop: "30px", padding: "8px 12px"}}
                                                className={`${styles.button1} ${styles.orange}`}
                                                onClick={() => handleSocialEdit(social.id, social)}><i
                                            className="fa fa-pencil" style={{marginRight: "5px"}}></i>
                                            Edit
                                        </button>
                                    </>
                                )}
